...
(function (global) {
    ...
    function getDxQueryDiffFromLevel(levelKey) {
        // v.wmc.pub diff 对应：2 Basic、3 Advance、4 Expert、5 Master
        const map = {
            basic: 2,
            advanced: 3,
            expert: 4,
    ...
    function updateAwmcConfirmLinks() {
        ...
        const anchors = document.querySelectorAll('a[href^="https://v.wmc.pub/?song="], a[href*="v.wmc.pub/?song="]');
        anchors.forEach(a => {
            const href = a.getAttribute('href');
            if (!href) return;
            try {
                const url = new URL(href);
                if (url.hostname !== 'v.wmc.pub') return;
